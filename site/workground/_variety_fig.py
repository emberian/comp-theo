...
d = json.load(open("atlas/variety.json"))
...
ax.set_title("Variety main effect across every instrument (3x3)\n"
...
for i, a in enumerate(agents):
    for j, w in enumerate(worlds):
        ...
        e = r["effect"]
        txt = f"{e:+.3f}\n[{r['lo']:+.2f},{r['hi']:+.2f}]"
        col = "white" if abs(e) > 0.45 * vmax else "black"
...
cb.set_label("variety main effect (success on - off)", fontsize=10)
fig.tight_layout()
fig.savefig("figs/atlas_variety.png", dpi=130)
print("WROTE figs/atlas_variety.png")
